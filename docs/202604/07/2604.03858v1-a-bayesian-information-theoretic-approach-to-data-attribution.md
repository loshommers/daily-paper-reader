...
## 摘要
训练数据归因 (TDA) 旨在将模型预测追溯到有影响力的训练样本，从而增强模型的可解释性和安全性。我们将 TDA 建模为一个贝叶斯信息论问题：根据子集引起的信息损失进行评分，即移除该子集时查询点熵的增加。这一准则将归因功劳分配给那些能够解决预测不确定性而非标签噪声的样本。为了扩展到现代网络，我们使用基于切线特征构建的高斯过程代理模型来近似信息损失。我们证明，该方法在单样本归因上与经典的影响分数一致，同时能促进子集的多样性。对于更大规模的检索，我们将其放宽为信息增益目标，并加入方差修正，以便在向量数据库中进行可扩展的归因。实验表明，该方法在反事实敏感性、基准真相检索和核心集选择方面具有竞争力的性能，证明了我们的方法在扩展到现代架构的同时，弥合了原则性度量与实践之间的鸿沟。
