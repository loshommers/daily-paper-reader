...
method: 客户端采用解耦架构并引入流匹配生成器替代特征提取器，服务端则利用合成样本驱动的验证方案和鲁棒聚合机制。
result: 在MNIST、FMNIST和CIFAR-10数据集上的实验表明，该方法在多种攻击场景下均实现了更高的准确率和更强的隐私保护。
conclusion: FedFG通过结合流匹配生成技术与解耦学习，成功平衡了联邦学习的准确性、隐私性与鲁棒性。
---
