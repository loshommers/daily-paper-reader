...
tldr: 随着基础模型规模扩大，AI安全威胁日益复杂且碎片化。本文针对数据与模型间相互依存的漏洞，提出了一个统一的闭环威胁分类框架。该框架从数据到数据、数据到模型、模型到数据及模型到模型四个维度系统地梳理了安全风险，揭示了威胁间的内在联系，为开发可扩展、跨模态的安全防御策略奠定了理论基础。
selection_source: fresh_fetch
motivation: 现有的机器学习安全研究大多孤立地对待威胁，缺乏一个统一的框架来揭示数据与模型之间相互交织的安全风险。
method: "提出一种统一的闭环威胁分类法，通过四个方向轴（D→D, D→M, M→D, M→M）来界定模型与数据间的交互威胁。"
result: 该框架成功将数据解密、投毒攻击、训练数据提取和模型抽取等多种孤立的威胁整合进一个互相关联的体系中。
conclusion: 该研究为基础模型时代下的安全分析提供了系统视角，有助于构建更具迁移性和跨模态的综合防御体系。
---

## 摘要
随着机器学习（ML）系统在规模和功能上的不断扩展，安全格局变得日益复杂，攻击与防御手段层出不穷。然而，现有研究大多孤立地对待这些威胁，缺乏一个连贯的框架来揭示其共同原理和相互依赖性。这种碎片化的视角阻碍了系统性的理解，并限制了全面防御方案的设计。至关重要的是，机器学习的两大核心资产——数据与模型——已不再相互独立；其中一方的漏洞会直接危及另一方。由于缺乏整体框架，关于这些双向风险如何在机器学习流水线中传播的问题仍悬而未决。为了填补这一关键空白，我们提出了一个统一的闭环威胁分类法，明确地沿着四个方向轴构建了模型与数据之间的交互。我们的框架为分析和防御基础模型提供了一个规范的视角。由此产生的四类安全威胁代表了不同但相互关联的攻击类别：(1) 数据→数据 (D→D)：包括数据解密攻击和水印去除攻击；(2) 数据→模型 (D→M)：包括投毒攻击、有害微调攻击和越狱攻击；(3) 模型→数据 (M→D)：包括模型逆向攻击、成员推理攻击和训练数据提取攻击；(4) 模型→模型 (M→M)：包括模型提取攻击。我们的统一框架阐明了这些安全威胁之间的内在联系，并为开发可扩展、可迁移且跨模态的安全策略奠定了基础，特别是在基础模型的背景下。
