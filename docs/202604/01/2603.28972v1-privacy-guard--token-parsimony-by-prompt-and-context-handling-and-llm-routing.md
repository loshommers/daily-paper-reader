...
title_zh: 通过提示词与上下文处理及大语言模型路由实现隐私保护与 Token 简约
...
## 摘要
大语言模型（LLM）的大规模采用迫使人们在运营成本（OpEx）与数据隐私之间进行权衡。当前的路由框架虽然降低了成本，但忽略了提示词的敏感性，使用户和机构面临向第三方云提供商泄露数据的风险。我们形式化了“不可分范式”：高级上下文管理在本质上与隐私管理相一致。我们提出了一种本地“隐私卫士”——这是一个由本地小语言模型（SLM）驱动的全方位上下文观察器——它执行抽象式摘要和自动提示词优化（APO），将提示词分解为聚焦的子任务，并将高风险查询重新路由至零信任或受保密协议（NDA）覆盖的模型。这种双重机制在消除敏感推理向量（零泄露）的同时，减少了云端 Token 负载（降低 OpEx）。一种基于后进先出（LIFO）的上下文压缩机制进一步限制了工作内存，从而限制了突发的泄露面。我们通过一个包含 1,000 个样本的数据集，在 2x2 基准测试（懒惰型 vs. 专家型用户；个人 vs. 机构秘密）上验证了该框架，实现了 45% 的综合 OpEx 降低，个人秘密的脱敏成功率达到 100%，并且通过 LLM-as-a-Judge 评估，APO 压缩后的响应相较于原始基准获得了 85% 的偏好率。我们的结果表明，Token 简约与零泄露在数学上是同一种上下文压缩算子的对偶投影。
