...
tldr: 针对跨医院放射影像数据共享中的隐私保护与数据效用平衡问题，本文提出了一种效用保持的去标识化流水线（UPDP）。该方法通过黑白名单过滤文本报告，并利用生成式过滤机制合成保留病理特征但去除隐私信息的影像。实验证明，该方法在保护隐私的同时，能维持模型在胸部X射线基准上的诊断准确性，并支持跨医院联合训练以提升性能。
selection_source: fresh_fetch
motivation: 现有的放射影像去标识化研究主要关注合规性，而忽略了去标识化数据在大型视觉语言模型训练和跨医院迁移中的效用保留问题。
method: 提出UPDP流水线，结合敏感词黑名单与病理词白名单处理报告，并采用生成式过滤机制合成保留诊断线索且去除身份信息的影像。
result: 实验显示，在去标识化数据上训练的模型保持了与原始数据相当的诊断精度，同时身份识别准确率显著下降，验证了隐私保护的有效性。
conclusion: UPDP为跨医院放射影像数据共享提供了一种安全且高效的方案，证明了去标识化数据在提升医疗AI模型性能方面的巨大潜力。
---
