...
tldr: 本研究针对持续学习中的灾难性遗忘问题，提出一种模块化架构。该架构通过任务特定专家和基于异常检测的门控机制实现参数隔离。核心创新在于“同步流水线”机制，在原始数据存在时并行完成教师学习、学生蒸馏和路由流形获取，并在学习后立即删除数据以保护隐私。利用紧瓶颈自编码器（TB-AE）在超高维空间中建立拓扑边界，实现无监督的任务发现与检索，有效解决了潜在空间拥挤问题。
selection_source: fresh_fetch
motivation: 旨在解决神经网络在顺序任务学习中的灾难性遗忘问题，并满足数据隐私保护和计算效率的需求。
method: 提出一种包含任务专家和TB-AE路由器的模块化架构，通过同步流水线进行实时蒸馏和无监督任务发现。
result: 在CV和NLP领域验证了该方法能有效区分高维流形，实现稳定的终身学习且无性能损失。
conclusion: 该研究证明了通过结构化参数隔离和实时蒸馏，可以在不保留原始数据的情况下实现高效、隐私合规的持续学习。
---

## 摘要
灾难性遗忘仍然是人工神经网络顺序任务学习中的主要障碍。我们提出了一种硅原生模块化架构，利用任务特定专家（Task-Specific Experts）和分布式、基于离群值的守门员（Gatekeeper）实现结构化参数隔离。超越传统的顺序巩固，我们的框架采用同步流水线（Simultaneous Pipeline），在原始数据存在于局部训练会话期间，并行进行教师学习、学生蒸馏和路由流形获取。这种方法确保了计算效率，并通过在任务学习完成后立即删除原始数据来符合 GDPR 等隐私指令。我们证明了紧瓶颈自编码器（TB-AE）可以有效区分高维潜空间中语义拥挤的流形，克服了标准变分方法固有的后验坍缩问题。通过建立严格的拓扑边界，我们的 TB-AE 解决了 4096 维大语言模型（LLM）嵌入中的潜空间拥挤问题，从而提供稳健的无监督新颖性信号。此外，我们验证了一种自主检索机制，该机制能够可靠地识别回归流形，从而在不产生冗余模块实例化的条件下实现稳定的终身学习。实验结果表明，我们的“实时蒸馏”（Live Distillation）方法起到了自然正则化器的作用，在计算机视觉和自然语言处理领域均实现了强大的记忆保留，且不存在学生保真度差距。
