...
tags: ["query:mu-ce"]
score: 6.0
evidence: 从LLM中蒸馏隐私敏感性评估能力
tldr: 本研究针对文本隐私评估中大语言模型（LLM）计算成本高且难以大规模处理敏感数据的问题，提出了一种知识蒸馏方法。通过将 Mistral Large 3 (675B) 的隐私敏感度评估能力迁移到仅 150M 参数的轻量级编码器模型中，并在涵盖10个领域的隐私标注数据集上进行训练。实验证明，该轻量级模型在保持与人类判断高度一致的同时，显著降低了计算开销，可作为去标识化系统的有效评估指标。
...
conclusion: 该研究证明了通过蒸馏 LLM 知识可以构建高效、精准且符合人类认知的隐私评估工具，为去标识化系统提供了实用的评估度量。
---

## 摘要
文本数据的准确隐私评估仍然是隐私保护自然语言处理中的一个关键挑战。最近的研究表明，大语言模型（LLMs）可以作为可靠的隐私评估器，与人类判断保持高度一致；然而，其计算成本以及在大规模处理敏感数据时的不切实际性限制了其在现实世界中的部署。我们通过将 Mistral Large 3 (675B) 的隐私评估能力蒸馏到参数量低至 150M 的轻量级编码器模型中，填补了这一空白。利用涵盖 10 个不同领域的隐私标注文本大规模数据集，我们训练了高效的分类器，这些分类器在大幅降低计算需求的同时，保持了与人类标注的高度一致性。我们在人类标注的测试数据上验证了我们的方法，并展示了其作为去标识化系统评估指标的实际效用。
