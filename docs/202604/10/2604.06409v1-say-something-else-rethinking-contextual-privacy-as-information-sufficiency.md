...
## 摘要
LLM 智能体越来越多地代表用户起草消息，然而用户经常过度分享敏感信息，且对于何为隐私存在分歧。现有系统仅支持抑制（省略敏感信息）和泛化（用抽象概念替换信息），且通常在单个孤立消息上进行评估，这使得策略空间和评估设置均不完整。我们将隐私保护型 LLM 通信形式化为一项信息充分性 (Information Sufficiency, IS) 任务，引入了自由文本假名化 (free-text pseudonymization) 作为第三种策略，即用功能等效的替代方案替换敏感属性，并提出了一种对话式评估协议，用于在现实的多轮追问压力下评估各项策略。在涵盖三种权力关系类型（机构、同伴、亲密）和三个敏感度类别（歧视风险、社会成本、边界）的 792 个场景中，我们从两个粒度的隐私性、隐蔽性和效用方面评估了七个前沿 LLM。结果表明，假名化在总体上实现了最强的隐私与效用权衡，而单消息评估系统性地低估了泄露风险，泛化策略在追问下其隐私性表现下降了多达 16.3 个百分点。
