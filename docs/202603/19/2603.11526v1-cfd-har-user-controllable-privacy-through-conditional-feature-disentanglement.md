...
tags: ["query:mu-ce"]
score: 7.0
evidence: 通过特征解耦进行隐私过滤以移除敏感信息
tldr: 针对可穿戴设备人体活动识别（HAR）中敏感信息泄露和标注样本有限的挑战，本文提出CFD-HAR框架。该方法利用条件特征解耦技术，在潜在空间中分离活动特征与敏感属性，实现了用户可控的动态隐私过滤。通过与自编码器少样本学习对比，研究揭示了隐私保护与数据效率间的权衡，为构建可信物联网智能系统提供了新方向。
selection_source: fresh_fetch
motivation: 旨在解决人体活动识别中传感器数据导致的隐私泄露风险，以及在有限标注样本下维持高性能的需求。
method: 提出一种基于条件特征解耦的表示学习技术，通过在潜在空间分离活动与敏感属性实现细粒度的隐私控制。
result: 实验表明CFD-HAR能提供显式且可调的隐私保护，而自编码器方法虽在少样本适应性上占优但缺乏隐私保障。
conclusion: 现有的单一范式无法完全满足下一代物联网HAR系统的需求，未来需探索兼顾隐私、少样本适应性和鲁棒性的统一框架。
---

## 摘要
现代可穿戴和移动设备配备了惯性测量单元（IMU）。运行在这些设备上的动作识别（HAR）应用利用基于机器学习的数据驱动技术来处理此类传感器数据。然而，基于传感器数据的 HAR 部署面临两个关键挑战：根据用户的隐私偏好保护嵌入在传感器数据中的敏感用户信息，以及在有限的标注样本下保持高识别性能。本文提出了一种通过细粒度特征解耦表示学习实现用户可控隐私的技术，用于动态隐私过滤。我们还将该技术的有效性与使用基于自动编码器表示学习的小样本 HAR 进行了对比。我们分析了它们的架构设计、学习目标、隐私保证、数据效率以及对边缘物联网（IoT）部署的适用性。研究表明，基于 CFD 的 HAR 通过在潜空间中分离活动属性和敏感属性，提供了显式且可调的隐私保护控制；而基于自动编码器的小样本 HAR 虽然具有更优的标签效率和轻量级适应性，但缺乏内在的隐私保护机制。我们进一步研究了这两种方法在持续物联网环境下的安全性影响，重点分析了它们在表示泄露和嵌入级攻击敏感性方面的差异。分析显示，单一范式都无法完全满足下一代物联网 HAR 系统的新兴需求。最后，我们概述了旨在共同优化隐私保护、小样本适应性和鲁棒性的统一框架研究方向，以实现可信的物联网智能。
