...
tags: ["query:mu-ce"]
score: 7.0
evidence: 神经元级梯度门控以减轻隐私风险并防止敏感数据泄露
tldr: 针对大型视觉语言模型（LVLM）在处理敏感信息时存在的隐私泄露风险，本文提出了 Neural Gate。该方法通过神经元级别的模型编辑，学习特征向量以定位与隐私概念相关的神经元，并精确引导参数更新。实验表明，Neural Gate 不仅能显著提高模型对已知和未知隐私查询的拒绝率，还能在增强隐私保护的同时保持模型在标准任务上的原始性能，解决了现有方法泛化性差和破坏性强的问题。
selection_source: fresh_fetch
motivation: 现有的隐私保护方法在处理未见过的隐私查询时泛化能力不足，且往往会损害模型在常规任务上的表现。
method: 提出一种名为 Neural Gate 的神经元级梯度门控方法，通过定位并编辑与隐私相关的神经元来增强模型的拒绝机制。
result: 在 MiniGPT 和 LLaVA 上的实验证明，该方法在显著提升隐私保护能力的同时，有效保留了模型的原有功能。
conclusion: Neural Gate 为缓解 LVLM 的隐私风险提供了一种兼具泛化性和非破坏性的有效解决方案。
---

## 摘要
大语言视觉模型（LVLMs）在广泛的视觉语言任务中展现出卓越的潜力，使其在金融和医疗等关键领域得到应用。然而，其日益广泛的部署也带来了显著的安全和隐私风险。恶意攻击者可能会利用这些模型提取敏感信息，凸显了一个关键漏洞。最近的研究表明，LVLMs 往往无法一致地拒绝旨在损害用户隐私的指令。虽然现有的隐私保护工作在防止敏感数据泄露方面取得了显著进展，但它们受到泛化性和非破坏性方面的限制。它们通常难以稳健地处理未见的隐私相关查询，并可能在无意中降低模型在标准任务上的性能。为了应对这些挑战，我们提出了 Neural Gate，这是一种通过神经元级模型编辑来缓解隐私风险的新方法。我们的方法通过提高模型对隐私相关问题的拒绝率来增强其隐私保护措施，关键在于将这种保护行为扩展到编辑过程中未遇到的新型敏感查询。Neural Gate 通过学习一个特征向量来识别模型对某一主题的表示中与隐私相关概念相关的神经元。这种定位随后精确地指导模型参数的更新。通过在 MiniGPT 和 LLaVA 上的全面实验，我们证明了我们的方法在保持模型原始效用的同时，显著提升了模型的隐私保护能力。
