...
tags: ["query:mu-ce"]
score: 6.0
evidence: 大模型驱动的文本个人隐私信息匿名化框架
tldr: 针对大模型时代敏感信息保护与数据可用性之间的矛盾，本文提出了一种基于本地大模型的匿名化框架。该框架通过将文本中的个人身份信息（PII）替换为类型一致的虚拟替代物，在确保数据不出域的前提下，保留了文本的流畅性与语义。实验证明，该方法在隐私保护、语义效用和下游任务训练能力方面均优于现有工业标准和前沿技术，为负责任的AI应用提供了有效方案。
selection_source: fresh_fetch
motivation: 在大模型应用中，如何在不损害数据实用性的前提下，有效防止敏感隐私信息的泄露是一个亟待解决的挑战。
method: 提出一种在本地运行的LLM驱动替换流水线，利用类型一致的虚拟数据替换PII，以实现隐私保护与语义保留。
result: 实验表明该方法在隐私性、语义一致性及下游模型微调效果上均达到SOTA水平，优于Presidio和Google DLP等基准。
conclusion: 本地LLM驱动的匿名化替换方案能够生成兼具安全性与操作价值的语料，适用于智能体问答和下游模型训练。
---
