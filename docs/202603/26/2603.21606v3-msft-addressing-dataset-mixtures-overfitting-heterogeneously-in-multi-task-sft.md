...
tldr: 针对多任务监督微调（SFT）中不同数据集学习动态不一致导致的过拟合问题，本文提出mSFT算法。该方法通过迭代搜索，识别并剔除最早过拟合的子数据集，并回滚至该任务的最优检查点继续训练。实验证明mSFT在多个基准和模型上均优于基准方法，能有效平衡任务间的学习差异，甚至在低计算预算下降低训练成本。
selection_source: fresh_fetch
motivation: 传统多任务SFT对所有数据集采用统一计算预算，导致学习较快的任务过早过拟合，而学习较慢的任务仍欠拟合。
method: 提出一种迭代式过拟合感知算法，通过动态识别并排除已过拟合的子任务，并回退至对应最优检查点来优化数据混合训练。
result: 在10个基准测试和6个基础模型上的评估显示，mSFT在性能上全面超越了4种基准方法，且对超参数不敏感。
conclusion: mSFT为多任务SFT提供了一种实用的过拟合感知方案，能够充分挖掘模型在多样化数据混合下的潜力。
---
