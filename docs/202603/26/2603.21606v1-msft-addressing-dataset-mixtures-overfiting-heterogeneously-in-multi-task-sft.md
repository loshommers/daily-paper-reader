...
title_zh: mSFT：解决多任务 SFT 中数据集混合异构过拟合问题
...
tags: ["query:mu-ce"]
score: 6.0
evidence: 识别并排除过拟合的子数据集
tldr: 针对多任务监督微调（SFT）中不同子数据集学习动态不一致导致的过拟合问题，本文提出 mSFT 算法。该方法通过迭代识别并剔除过拟合任务，并回退至最优检查点继续训练，有效解决了传统统一计算预算下的性能瓶颈。实验证明 mSFT 在多个基准测试和模型上均优于现有基准，且能在提升性能的同时降低训练成本。
selection_source: fresh_fetch
motivation: 传统多任务 SFT 采用统一计算预算，导致学习较快的任务过早过拟合，而较慢的任务仍欠拟合。
method: 提出一种迭代式过拟合感知搜索算法，动态识别并排除已过拟合的子数据集，并回退到该任务的最优检查点继续训练。
result: 在 10 个基准测试和 6 个基础模型上，mSFT 性能一致优于 4 种基准方法，且对超参数不敏感。
conclusion: mSFT 为多任务 SFT 提供了一种实用的过拟合感知方案，能有效最大化模型在多样化数据混合中的潜力。
---

## 摘要
当前的语言模型训练通常在所有子数据集上使用统一的计算预算进行多任务监督微调（SFT）。这种方法从根本上说是次优的：异构的学习动态导致学习较快的任务过早过拟合，而学习较慢的任务仍处于欠拟合状态。为解决这一问题，我们引入了 mSFT，这是一种针对多任务数据混合的迭代式、过拟合感知的搜索算法。mSFT 在活跃的混合数据上训练模型，识别并排除最早过拟合的子数据集，并在继续训练前回退到该特定的最优检查点。广泛的评估表明，mSFT 在 10 个基准测试和 6 个基础模型上始终优于 4 个基准方法。进一步分析证实，mSFT 在不同的数据集规模、任务粒度下保持了稳健的增益，且对其唯一的新超参数（计算预算）不敏感。值得注意的是，在低计算预算下，mSFT 可以在降低训练 FLOPs 的同时提高性能。最终，mSFT 为多任务 SFT 建立了一种实用的过拟合感知算法，最大限度地发挥了模型在多样化数据混合中的潜力。
