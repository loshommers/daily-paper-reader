...
tags: ["query:mu-ce"]
score: 7.0
evidence: 身份抑制网络使身份匿名化
tldr: 针对视频面部表情识别中的身份隐私泄露问题，本文提出一种无需身份标签的开放集隐私保护框架。通过利用视频内外的知识先验构建身份抑制网络，在匿名化身份的同时保留表情特征，并结合去噪模块恢复识别性能。该方法在三个数据集上证明了其在保护隐私的同时，能保持与有监督方法相当的表情识别精度，为开放场景下的隐私保护提供了新思路。
selection_source: fresh_fetch
motivation: 现有的隐私保护表情识别方法在缺乏身份标签的开放集视频场景下，难以有效解耦身份与表情信息并保护隐私。
method: 提出一种两阶段框架，利用视频知识先验训练身份抑制网络进行匿名化，并配合去噪模块恢复表情相关信息。
result: 在三个视频数据集上的实验表明，该方法在无需身份标注的情况下，实现了与身份监督方法相媲美的隐私保护效果和识别准确率。
conclusion: 本研究证明了利用知识先验可以在无监督开放集环境下，有效实现身份脱敏与表情识别性能之间的平衡。
---

## 摘要
面部表情识别（FER）依赖于面部数据，这些数据本质上会暴露身份，从而引发严重的隐私担忧。当前的隐私保护方法在现实的开集视频场景中通常会失效，因为在这些场景中身份是未知的，且无法获得身份标签。我们针对具有挑战性的开集设置提出了一种基于视频的隐私保护 FER 两阶段框架，该框架在任何阶段都不需要身份标签。为了解耦隐私和效用，我们首先利用从无身份标签的真实视频中提取的视频内和视频间知识先验，训练一个身份抑制网络。该网络在匿名化身份的同时保留了表情线索。随后的去噪模块恢复了与表情相关的信息，并有助于恢复 FER 性能。此外，我们引入了一种基于证伪的验证方法，该方法利用识别先验在不需要标注身份标签的情况下严格评估隐私鲁棒性。在三个视频数据集上的实验表明，我们的方法在有效保护隐私的同时，保持了与受身份监督的基准模型相当的 FER 准确率。
