...
## 摘要
联邦学习支持在不共享原始数据的情况下进行协作训练，但在客户端异构性和流式分布偏移（其中漂移和新数据会损害收敛并导致遗忘）的情况下面临挑战。我们提出了一种联邦联想记忆框架，用于在异构、持续的环境中学习共享原型，其中客户端数据是独立的，但不一定平衡。每个客户端将其经验编码为低秩赫布算子，并发送到中央服务器进行聚合和分解为全局原型。该方法保护了隐私，避免了集中式重放缓冲区，并且对小型、噪声或演化的数据集具有鲁棒性。我们将聚合建模为一个低秩加噪声的谱推理问题，推导出了可检测性和检索鲁棒性的理论阈值。一个基于熵的控制器在流式机制中平衡了稳定性与塑性。在异构客户端、漂移和新颖性场景下的实验表明，该方法改进了全局原型重构和联想检索，支持了联邦整合的谱视角。
